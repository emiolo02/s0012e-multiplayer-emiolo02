...
namespace Physics {
    ...
    */
    ...
    assert(vbAccessor.type == fx::gltf::Accessor::Type::Vec3 || vbAccessor.type == fx::gltf::Accessor::Type::Vec4);
        ...
        // HACK: Assumes 3d or 4d vertex positions
        ...
        mesh->bSphereRadius = std::max(mesh->bSphereRadius, std::abs(vbAccessor.min[0]));
        mesh->bSphereRadius = std::max(mesh->bSphereRadius, std::abs(vbAccessor.min[1]));
        mesh->bSphereRadius = std::max(mesh->bSphereRadius, std::abs(vbAccessor.min[2]));
    }
